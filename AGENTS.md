...
GitHub Actions workflow `.github/workflows/free-threaded-smoke.yml` runs on every push/PR: installs 3.14t, verifies no-GIL imports, compiles all source, runs concurrency smoke tests, and compares GIL vs no-GIL benchmark throughput. `quick-check.yml` additionally builds the native extension and runs the full pytest suite against it; `skia-wheels.yml` builds the release wheels; `docker.yml` builds the image with the wheel baked in (and stays green without one).
