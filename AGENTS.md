...
- `src/sekai/profile/custom_profile/skia.py` — no plot.py tree exists to lower: the layout carrier is the Unity
  ...
  TMP texts retain Python's TMP layout oracle but have three pixel paths. Strict plain text uses IR Text; every
  other TMP/SDF element is offered to the sparse path before any compatibility raster. Legacy/unsupported glyphs
  use `SdfQuad` (capability 9): Python ships a warped A8 field and Rust shades it. Static atlas glyphs use
