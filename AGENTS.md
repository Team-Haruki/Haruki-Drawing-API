...
- A separate **isolated subprocess pool** (`src/core/heavy_render_pool.py`) runs only the two crash-prone heavy tasks (`HeavyTaskKind = "deck_recommend" | "chara_birthday"`), with its own queue limit and hard timeout. Everything else stays in-process. The backend-neutral `EncodedImagePayload` in `src/core/image_payload.py` is the common return type of the Skia `try_render_*_payload` functions; `heavy_render_pool.py` only keeps a compatibility re-export.
...
- `src/sekai/profile/custom_profile/skia.py` — no plot.py tree exists to lower: the layout carrier is the Unity
  ...
  Decorative TMP texts retain Python's TMP layout oracle but have three pixel paths. Legacy/unsupported content
  uses `SdfQuad` (capability 9): Python ships a warped A8 field and Rust shades it. Static atlas glyphs use
  `SdfAtlasQuad` (capability 18), and dynamic/fallback source-font glyphs use `SdfFontQuad` (capability 19): Rust
  owns field generation, Pillow-compatible L/BICUBIC resize/warp, shading, and the source-font SDF cache; Python
  ships only controlled asset/font geometry and the affine plan. The scalar derivation exists once
  ...
  `scripts/gen_sdf_quad_golden.py` whenever the shading math changes).
...
**Capability handshake.** The extension exports `IR_CAPABILITY` (currently **19**) and `RAW_BUFFER_CAPABILITY`;
`src/sekai/skia_renderer/canvas.py` checks the former against `REQUIRED_NATIVE_IR_CAPABILITY` (also 19). A too-old
