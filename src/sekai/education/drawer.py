...
from src.sekai.base.configs import ASSETS_BASE_DIR, DEFAULT_FONT, DEFAULT_BOLD_FONT
                ...
                for idx, challenge in enumerate(character_challenges):
                    ...
                    with HSplit().set_content_align("c").set_item_align("c").set_sep(8).set_h(row_h).set_padding(4).set_bg(roundrect_bg(fill=bg_color)):
                        ...
                        with Frame().set_w(w4).set_content_align("lt"):
                            ...
                            color = (255, 50, 50, 255)
                            if x > 2500000:    color = (100, 255, 100, 255)
                            elif x > 2000000:  color = (255, 255, 100, 255)
                            elif x > 1500000:  color = (255, 200, 100, 255)
                            elif x > 1000000:  color = (255, 150, 100, 255)
                            elif x > 500000:   color = (255, 100, 100, 255)
                    ...
                    with Grid(col_count=2).set_content_align("l").set_item_align("l").set_sep(20, 4).set_padding(16):
                        for bonus in bonuses_group:
                            ...
                            with HSplit().set_content_align("l").set_item_align("l").set_sep(4):
                                if chara_icon:
                                    ImageBox(chara_icon, size=(None, 40))
                                TextBox(f"{bonus.total:.1f}%", header_style).set_w(100).set_content_align("r").set_overflow("clip")
                                detail = f"区域道具{bonus.area_item:.1f}% + 角色等级{bonus.rank:.1f}% + 烤森玩偶{bonus.fixture:.1f}%"
            ...
            with HSplit().set_content_align("lt").set_item_align("lt").set_sep(16).set_bg(roundrect_bg()).set_padding(8):
                for item in area_items:
                    ...
                    with VSplit().set_content_align("l").set_item_align("l").set_sep(8).set_item_bg(roundrect_bg()).set_padding(8):
                        # 列头
                        item_icon = await get_img_from_path(ASSETS_BASE_DIR, item.item_icon_path) if item.item_icon_path else None
                        target_icon = await get_img_from_path(ASSETS_BASE_DIR, item.target_icon_path) if item.target_icon_path else None
                ...
                for idx, info in enumerate(leader_counts):
                    ...
                    with HSplit().set_content_align("c").set_item_align("c").set_sep(8).set_h(row_h).set_padding(4).set_bg(roundrect_bg(fill=bg_color)):
                        ...
                        with Frame().set_w(w5).set_content_align("lt"):
                            ...
                            color = (255, 50, 50, 255)
                            if pc > 50000:    color = (100, 255, 100, 255)
                            elif pc > 40000:  color = (255, 255, 100, 255)
                            elif pc > 30000:  color = (255, 200, 100, 255)
                            elif pc > 20000:  color = (255, 150, 100, 255)
                            elif pc > 10000:  color = (255, 100, 100, 255)
