...
# 合成自定义房间控分图片
async def compose_custom_room_score_control_image(
            ...
            with HSplit().set_content_align('lt').set_item_align('lt').set_sep(hsep):
                ...
                with VSplit().set_content_align('c').set_item_align('c').set_sep(vsep):
                    ...
                    for i, (_, event_bonus) in enumerate(rqd.candidate_pairs):
                        bg = bg_fn(i + 1)
                        TextBox(f"{event_bonus} %", style2).set_size((w1, gh)).set_content_align('c').set_padding((16, 0)).set_bg(bg)
                # 歌曲
                with VSplit().set_content_align('c').set_item_align('c').set_sep(vsep):
                    TextBox("可用歌曲", style1).set_size((w2, gh)).set_content_align('c').set_bg(bg_fn(0))
                    for i, (event_rate, _) in enumerate(rqd.candidate_pairs):
                        bg = bg_fn(i + 1)
                        with HSplit().set_content_align('c').set_item_align('c').set_sep(4).set_padding((8, 0)).set_size((w2, gh)).set_bg(bg):
                ...
                with VSplit().set_content_align('c').set_item_align('c').set_sep(vsep):
                    ...
                    for i, (event_rate, _) in enumerate(rqd.candidate_pairs):
                        bg = bg_fn(i + 1)
                        TextBox(f"{event_rate}", style2).set_size((w3, gh)).set_content_align('c').set_padding((8, 0)).set_bg(bg)
...
# 合成歌曲meta图片
async def compose_music_meta_image(
    ...
    with Canvas(bg=SEKAI_BLUE_BG).set_padding(BG_PADDING) as canvas:
        with HSplit().set_content_align('lt').set_item_align('lt').set_sep(8):
            for rqd in requests:
                ...
                with VSplit().set_content_align('lt').set_item_align('lt').set_sep(8).set_bg(roundrect_bg(alpha=80)).set_padding(16):
                    # 歌曲标题
                    with HSplit().set_content_align('l').set_item_align('l').set_sep(4):
                        ImageBox(music_cover, size=(48, 48), use_alpha_blend=False)
                        TextBox(f"【{rqd.music_id}】{rqd.music_title}", TextStyle(font=DEFAULT_BOLD_FONT, size=24, color=BLACK))
                    ...
                    with VSplit().set_content_align('lt').set_item_align('lt').set_sep(8).set_item_bg(roundrect_bg(alpha=80)):
                        for meta in rqd.metas:
                            with VSplit().set_content_align('lt').set_item_align('lt').set_sep(8).set_padding(8):
                                ...
                                multi_score = meta.base_score + sum(meta.skill_score_multi) * multi_skill + meta.fever_score * 0.5 + 0.01875
    ...
    # 计算每列宽度
            ...
            with HSplit().set_content_align('c').set_item_align('c').set_sep(hsep):
                ...
                with VSplit().set_content_align('c').set_item_align('c').set_sep(vsep):
                    ...
                    for i, row in enumerate(rqd.items):
                        bg = row_bg_fn(i + 1)
                        with HSplit().set_content_align('l').set_item_align('l').set_sep(4).set_padding((8, 0)).set_size((w, gh)).set_bg(bg):
                ...
                with VSplit().set_content_align('c').set_item_align('c').set_sep(vsep):
                    ...
                    for i, row in enumerate(rqd.items):
                        ...
                        TextBox(f"{row.level}", TextStyle(DEFAULT_BOLD_FONT, 20, WHITE)) \
                            .set_size((w, gh)).set_content_align('c').set_bg(diff_bg)
                ...
                def add_text_column(header_text, value_getter):
                    ...
                    current_col_idx += 1
                    with VSplit().set_content_align('c').set_item_align('c').set_sep(vsep):
                        TextBox(header_text, title_style).set_size((w_col, gh)).set_content_align('c').set_bg(row_bg_fn(0))
                        for i, row in enumerate(rqd.items):
                            bg = row_bg_fn(i + 1)
                            TextBox(value_getter(row), item_style).set_size((w_col, gh)).set_content_align('c').set_bg(bg)
