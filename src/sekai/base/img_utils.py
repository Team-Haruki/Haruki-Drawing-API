...
def save_transparent_static_gif(img: Image, save_path: str, alpha_threshold: float = 0.5):
    ...
    while True:
        ...
        transparent_color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
        ...
        img.save(
            save_path, save_all=True, append_images=[img], duration=100, loop=0, transparency=transparent_color_index
        )
        break
    """
...
def save_apng(images: List[Image.Image], save_path: Union[str, Path], duration: int = 50, loop: int = 0) -> None:
    ...
    rgba_images[0].save(
        save_path, format="PNG", save_all=True, append_images=rgba_images[1:], duration=duration, loop=loop
    )
