...
async def _compose_vlive_entry_image(
    ...
    title_style = TextStyle(font=DEFAULT_BOLD_FONT, size=20, color=(24, 24, 28))
    ...
    bg_fill = (255, 244, 251, 178) if vlive.living else (255, 248, 253, 168)

    with Canvas().set_padding(0) as canvas:
        with Frame().set_w(760).set_padding(18).set_bg(roundrect_bg(bg_fill, radius=16)):
                ...
                with HSplit().set_padding(0).set_sep(16).set_item_align("lt").set_content_align("lt"):
                    with VSplit().set_padding(0).set_sep(12).set_item_align("l").set_content_align("l"):
                        if banner is not None:
                            ImageBox(banner, size=(None, 100), use_alpha_blend=True)
