...
- **对拍**：`scripts/skia_parity_sweep.py` 当前有 **65 个可渲染 payload 用例，65 ok / 0 失败**，另有
  ...
  三角背景的散布现在**只在 Python 生成一次**（`base/triangle_bg.py`，blake2b + 整点量化种子），
...
| 端点 | 缓存形态 | 对 Skia 的含义 |
|---|---|---|
| `honor` | 整图结果缓存（`get_composed_image_cached` mem TTL，无 disk 层） | Skia 侧另有 payload 缓存（`payload_cache.py`，键 = Pillow 缓存键 + `\|skia\|格式\|质量\|wm:{水印文本}`——水印里的 `DT` 精确到秒，所以 `dt` 一变就 miss） |
| ~~`card/list`、`card/box`~~ | **整页缓存已于 2026-07-14 删除**（`f088c41`） | 两者把墙钟画进画布（`add_request_watermark` 的 `DT:` 页脚；card/list 还按 `request_now()` 判「未上线」），而 key 里只有 request——命中必然发陈旧图。key 诚实了就永远命不中（`dt` 是毫秒墙钟），所以缓存本身才是 bug。由 `tests/test_asset_signature_cache_key.py::test_card_pages_are_not_cached` 钉死 |
...
| 背景 | ImageBg fit/fill/fixed/repeat + 9 向对齐 + `GaussianBlur(3)` + 压暗；后续已改由 `Painter.image_bg` 发普通 `Image(path)` 节点（`catmull_rom` + multiply tint + `blur_sigma`），可保持 `AssetImageRef` 惰性；TriangleBg `main_hue` 自定义色相 + `size_fixed_rate` |
...
| 项 | 内容 | 验证 |
|---|---|---|
| **新原语**：Image `source_rect` | Image 节点源像素裁剪窗 `[x0,y0,x1,y1]`，在 fit 之前应用（对应 Pillow `img.crop(box)`）；现已从 IR 节点上提到 `ImageBox` 与三种 `Painter.paste*` 的共享 `source_rect` 参数，并可配显式 `sampling`。production-local MySekai site crop 已消费该公共面 | 像素级（裁剪选对象限、均匀） |
| **新原语**：`TintMode::recolor` | 保留源 alpha 作蒙版、RGB 替换为常量色（SrcIn），色 alpha 缩放结果 alpha；区别于 multiply/mix。共享 `ImageTint` 公共面提供 `multiply|recolor`，production-local MySekai site/spawn 已分别消费 | 像素级（设 RGB、保 mask；alpha 缩放） |
...
| `adjust_image_alpha_inplace` | `Image.alpha` 仅 multiply | ➖ helper **仍在**（`base/img_utils.py`），并非已删除 |
| `center_crop_by_aspect_ratio`（纯裁剪） | `Image` fit=cover（裁剪 + 缩放） | ✅ `fit=crop` 纯裁剪 + `Image.source_rect` 源像素裁剪窗 |
| `ImageBg`（站点背景） | 仅全屏 cover + NEAREST 采样 | ✅ `Painter.image_bg` 支持 fit/fill/fixed/repeat + 9 向对齐；`IRPainter` 发普通 `Image(path)` 节点，以 `catmull_rom`、multiply tint（`floor((1-fade)*255)`）与 source→destination 缩放的 `blur_sigma` 表达压暗/`GaussianBlur(3)`，因此默认 fade/blur 也保持 ref 惰性（capability 10） |
...
- `add_watermark_to_image` 的**光栅页脚**（扩展画布 + 采样底部条作页脚底 + 多行右对齐 + 手工阴影）已在
  Skia 侧复刻：`chart` 与 `honor` **手写页脚外壳的 IR**（不是手写布局——honor 的徽章本体是
  `splice_root_children()` 拼进来的 widget 树 IR），单次原生渲染即可（`SelfImage` 节点在原生侧采样
  已渲染的画布取底部条拉伸作页脚底 + 右对齐阴影文字），Python 全程不碰像素。
  见 `src/sekai/chart/drawer.py`、`src/sekai/honor/skia.py`。
