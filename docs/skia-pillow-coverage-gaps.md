...
> **最终覆盖（2026-06-24，按 16 路逐端点审计核实）**：53 个 `@router.post` 绘图端点中
> **50 个已接 Skia 影子层**（门控 `use_skia_plot`，默认关，含逐端点 Pillow 回退）。仅 3 个未接：
> `profile/custom-profile-card`（独立 Unity/TMP-SDF 栅格器，无 Canvas 可渲染）、`chart`
> （独立 `pjsekai_scores_rs` Rust crate）—— 二者**无 plot.py widget 树**故根本不在影子层迁移范围内；
> `honor`（徽章合成,全 mem+disk 缓存,bonds 分支才需 alpha-mask 原语,刻意留 Pillow）。
> 即**所有有 widget 树的可迁移端点已 100% 接入**。
...
| **真热点**：mysekai msr_map 单图 | 最后一个每请求、未缓存、纯 Pillow 的整图场景。单图 build 改返回 Canvas（而非 `get_img()`），经共享 IRPainter 路径走原生并行渲染；裁剪/着色/重着色经现有 mem 图传输。逐尺寸一致 mean 0.27、p99=0 | Skia vs Pillow 对拍 |
| **关闭回退**：渐变文字 | `IRPainter.text` 原本对渐变填充抛 `SkiaUnsupported` → 整张画布回退 Pillow（任何 `DIFF_COLORS["append"]` 难度标签触发）。改为把渐变端点映射到字形 overlay（文字墨水尺寸 + 10px，锚于绘制位、cjk_top），复用 `_fill()`。Rust Text 节点本就渲染渐变 | 视觉一致（紫→粉对角），mean 3.4（字形栅格差异，非映射 bug） |
| **收尾**：sk player-trace/rank-trace | 见上方端点表；最后两个无 Skia 路径的 sk 端点 | mean<1 |

刻意不做（边际/已覆盖）：`gacha` 的 `concat_images([star]*n)` → HSplit（gacha 已走 Skia，星条本就当 mem 图传，仅省一次微小 concat；多 call-site 改造风险 > 收益）；msr_map 多图网格转原生子组（已作单张 mem 图高效传输）。两者均属微优化。

超范围（保持 Pillow，审计确认）：`profile/custom_profile`（~10.5k 行 Unity 保真栅格器，SDF/TMP/仿射/blend，XL，零上行价值）；`honor`（mem+disk 全缓存，bonds 分支才需 alpha-mask+裁剪，留 mem 图）；卡缩略图/profile 模块预渲染（磁盘缓存，留 mem 图）；GIF/APNG 导出；`chart`（已是独立 Rust crate）；Tier-2 原生图表。
...
| `card/detail` | ✅ Skia(影子层) | 进度审计发现的唯一真缺口(plot.py widget 树,早期随 list/box 直迁时漏掉);split 为 `_build_card_detail_canvas` + 影子层。布局逐尺寸一致;文字密集行(长 CJK+Latin 混排)字形步进累积差略高于稀疏端点,但视觉一致、默认门控关 |
...
| `sk`(line/query/check-room/csb/speed/winrate) | ✅ Skia(影子层) | W1;6 端点。含 scale 1.5/2.0 与 csb **条件 scale**(`1.5 if <10 else 1.0`,build 返回 (canvas, scale) 元组)。抽检 query(1.5×)、csb 大小两档、均逐尺寸一致 |
| `sk`(player-trace/rank-trace) | ✅ Skia(影子层) | 剩余迁移批次;matplotlib 仍渲染图表位图,外壳(圆角白卡 + WL 图标列 + 水印)走影子层,位图当 mem 图传。逐尺寸一致 mean<1。Tier-2 原生图表(双轴/AutoDateLocator/axvspan 等)仍排除 |
| `misc/chara_birthday` | ✅ Skia(影子层,heavy worker) | W1;Canvas bg 是 `ImageBg(运行时卡图)`→ mem 图。heavy worker 先试 Skia。抽检逐尺寸一致(672×752) |
| `misc/alias_list` | ✅ Skia(影子层) | 收尾;有最终结果缓存(PIL 存不下 payload → Skia 路径复用 `_skia_payload_cache`,键同 Pillow 缓存键)。运行时 alpha-trim 剪影走 mem 图。逐尺寸一致(1185×433),结果缓存命中已验证 |
| `gacha`(list/detail)、`event/list`、`vlive/list` | ✅ Skia(影子层) | W6 图中图。**无需新缓存策略**:条目本就是 `ImageBox(缓存PIL)`,影子层把缓存条目当 mem 图传,每条目 mem+disk 缓存原封不动。逐尺寸一致。实测(20 条目暖缓存)Skia 全胜:vlive **2.46×**、gacha 1.60×、event/list 1.38×(最贴图主导,边际最小但仍赢);并发 K8 优势放大(vlive ~4×、gacha ~2.6×、event ~1.8×)。故 Phase 2(内容 hash mem 缓存)**不需要** |
| `profile/custom`、`chart`、`honor` | ❌ 排除/特殊 | 详见下方"排除" |
...
| 能力 | 状态 |
|---|---|
| `multiply_image_by_color`（乘法 tint） | ✅ `Image.tint` mode=`multiply`（`ir_builder.image_tint`，2026-06-24 批次） |
| `mix_image_by_color`（向色彩 alpha 加权 lerp） | ✅ `Image.tint` mode=`mix` |
| alpha→纯色重着色（保 alpha、RGB 替常量，SrcIn） | ✅ `Image.tint` mode=`recolor`（剩余迁移收尾批次） |
| 源像素裁剪窗（`img.crop(box)`） | ✅ `Image.source_rect [x0,y0,x1,y1]`（同上） |

> 仅「任意图片 alpha 作蒙版叠到另一组」（`putalpha(mask.split()[3])`）仍未做 ——
> 唯一消费者是 honor bonds（全缓存、超范围），有更高价值消费者再建 alpha-mask-through-image。
