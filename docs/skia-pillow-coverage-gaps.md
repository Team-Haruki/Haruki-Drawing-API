...
| `event`(detail/record) | ✅ Skia(影子层) | W4;`detail` 抽检逐尺寸一致(1638×1020,内嵌 profile card + 4 卡缩略图经 mem 图)。`planner` 委托 deck(待迁),`list` 属 W6 |
...
| `score`(W2–W3) | ⏳ 待迁 | 纯 widget 树,仅需影子层(`music/detail` 已随 music 迁) |
| `deck / gacha / event-list / vlive`(W5–W6) | ⏳ 待迁 | deck/recommend 是 event/planner 的依赖;+ 缓存策略 |
