...
- **绝大多数端点已收敛为一棵树**：它们用 `src/sekai/base/plot.py` 的 widget 树描述页面；Pillow 走
  ...
  **专用 scene builder 已退役**（`card_render.py` 删除，见下文条目 9），两者现在都画 card drawer 的共享
  widget 树。
- **例外：honor 与 chart 仍手写 IR**（`src/sekai/honor/skia.py`、`src/sekai/chart/drawer.py`）。这两处直接用
  `IRBuilder` 按绝对坐标 emit IR v2 节点，**完全不经过 plot.py 的 widget 树，也不经过 `IRPainter`**（两个文件
  见 `honor/skia.py` 的模块注释;chart 无模块注释,证据是 `chart/drawer.py:24,208` 直接构造 `IRBuilder`,
  全文件不出现 plot/Canvas/IRPainter）。风险不对称：
  - **honor 是真正的两套布局**：Skia 走 `honor/skia.py` 的手写 IR，Pillow 走 `honor/drawer.py`
    `_compose_full_honor_image_sync` 的独立 composer，两边各自算画布尺寸、crop 窗口和文字位置。这正是
    `card_render.py` 当初的那类 **drift 风险**（改一边忘另一边 → 两后端输出静默分叉），只是这次还没结清。
  - **chart 的手写 IR 只是薄信封**：谱面栅格由 `pjsekai-scores-rs` 产出，IR 只负责把它 + 水印页脚拼起来；
    Pillow 回退路径（`compose_music_chart_image` → `add_request_watermark_to_image`）共用同一批
    `get_watermark_render_spec` 度量，重复的布局面积很小。
...
绝大多数端点走上面那条共享 widget 树；**honor 与 chart 例外**，它们手写 IR，不经过 `plot.py` 也不经过
`IRPainter`（见「当前状态」的例外说明）。
...
flowchart LR
    ...
    A2["/honor, /chart (例外)"] --> I["IRBuilder 手写 IR 场景"]
    I --> E
    A2 -- "回退" --> P2["honor: _compose_full_honor_image_sync (纯 Pillow)<br/>chart: crate PNG + PIL 水印"]
    P2 --> H
```
...
- 审计当时 Card List 仍是独立手写 IR builder（两套布局）；**Card List 这一处已在同日的条目 9 结清**——它现在
  与 card/box 一样共享 widget tree。但**手写 IR 并未在全项目消失**：`honor/skia.py` 与 `chart/drawer.py` 至今
  直接用 `IRBuilder` 发 IR，不走 widget tree / `IRPainter`；其中 honor 同时还有 `honor/drawer.py` 的独立 Pillow
  composer，是与 Card List 同类的两套布局 drift 风险（见「当前状态」）。
