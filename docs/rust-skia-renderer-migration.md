...
- 少量 builder 仍先用 `get_img_from_path()` 解码 PIL Image；pristine provenance 让 IRPainter 最终发 asset
  ...
  event、gacha 与 chara-birthday 的背景均已改传 `AssetImageRef`。部署本地的 proprietary
  `mysekai/drawer.real.py` 若仍 eager 加载背景，那只是尚未同步的调用点，不是架构要求。
...
7. ✅ 其余 builder 迁 `get_asset_image_ref`（10 文件并行迁移 + 每文件双路对抗复核）：card 8 处、
   ...
   **仍刻意保留 `get_img_from_path` 的位置**（改了就是 bug，勿"顺手修"）是任何走 PIL 像素 API 的：
   card `_circular_progress_avatar`(convert/resize/mask)、gacha `concat_images`、
   ...
   **2026-07-18 续补**：原来列在这里的 `ImageBg(fade>0/blur)` 例外已失效。`ImageBg.__init__` 不再
   ...
   gacha detail、chara-birthday 背景已转 `AssetImageRef`；本地 `drawer.real.py` 的 MySekai 背景调用点可独立续迁。
...
| Lazy AssetRef | Done | 仓库内 builder 已迁到 `get_asset_image_ref`（card / gacha / event / score / vlive / misc / stamp / profile / inventory / music …）；只有走 PIL 像素 API 的位置刻意保留 `get_img_from_path`。`ImageBg(fade/blur)` 已由 `Painter.image_bg` 惰性表达，不再是 eager 例外；本地 proprietary MySekai 实现需按部署版本另审。 |
