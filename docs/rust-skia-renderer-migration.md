...
- **绝大多数端点已收敛为一棵树**：它们用 `src/sekai/base/plot.py` 的 widget 树描述页面；Pillow 走
  ...
  **专用 scene builder 已退役**（`card_render.py` 删除，见下文条目 9），两者现在都画 card drawer 的共享
  widget 树。
- **所有拥有 widget 树的布局都只剩一棵树**：honor 的两套布局已结清（见下文条目 12）。
...
- **图片 lazy 边界已继续下沉**：`ImageBox` 与三种 `Painter.paste*` 共享 `source_rect`、显式 sampling 和
  ...
  thumbnail 与 profile X icon 已改为 path ref。map/map_multi IR mem 分别约下降 92.3%/85.6%；harvest
  point 亦已于 2026-07-19 ref 化（512² 探测走 `AssetImageRef.size`，长边算术 Python 复算 +
  `ImageBox(size, sampling="linear")`，Pillow 基线 max_delta=0），此后 map/map_multi 的 IR mem
  归零（8 项/465KB 与 42 项/2.5MB → 各 0）。
...
- 少量 builder 仍先用 `get_img_from_path()` 解码 PIL Image；pristine provenance 让 IRPainter 最终发 asset
  path，但 Python 的首次解码已经发生。**只有确实调用 PIL 像素 API 的位置才是刻意保留**（清单见条目 7）。
  `ImageBg(fade/blur)` 已不在此列：效果移入 `Painter.image_bg` 后，widget 构造不再物化像素；card 三处、
  event、gacha、chara-birthday 与部署本地 proprietary `mysekai/drawer.real.py` 的资源背景均已改传
  `AssetImageRef`。
...
7. ✅ 其余 builder 迁 `get_asset_image_ref`（10 文件并行迁移 + 每文件双路对抗复核）：card 8 处、
   ...
   **仍刻意保留 eager 的边界**（勿"顺手修"）包括真实走 PIL 像素 API 的 card
   `_circular_progress_avatar`（convert/resize/mask）、gacha `concat_images` / 缺图 fallback、honor 的
   mask/尺寸合成输入、misc alias silhouette alpha-trim，以及 mysekai harvest point（`.size` 探测 512²
   回退规则；**该项已于 2026-07-19 ref 化**——`AssetImageRef.size` 本身就是读头探测，规则无需像素）；
   另一处刻意保留：mysekai phenom 图标（50² **LANCZOS** 重采样 + 禁用态 ImageDraw 划线改像素——
   LANCZOS 不在 `ImageSampling` 里，转换要么加采样模式（capability bump）要么接受 catmull_rom 漂移，
   现状收益不值）。本轮另确认两处双阶段重采样边界，见下。
   **2026-07-18 续补**：原来列在这里的 `ImageBg(fade>0/blur)` 例外已失效。`ImageBg.__init__` 不再
   resolve/filter；`Painter.image_bg` 在 Pillow replay 时保留旧的 blur→brightness→resize 顺序，
   `IRPainter` 则发普通 `Image(path)` 节点，以 `catmull_rom` 采样、向下取整的 multiply tint 和按
   source→destination 比例缩放的 `blur_sigma` 表达效果（capability 10）。card detail/list/box、event detail、
   gacha detail、chara-birthday 与本地 `drawer.real.py` 的 MySekai 资源背景均已转 `AssetImageRef`。
   **同日续补图片装饰公共面**：`ImageBox` 与三种 `Painter.paste*` 统一接受 `source_rect`（源像素 xyxy，
   resize 前裁剪）、`sampling`（`nearest|linear|catmull_rom`）和 `ImageTint`（`multiply|recolor`）；Pillow
   replay 与 IRPainter 消费同一组参数，未新增 IR 字段或 capability。production-local MySekai site 图因此改为
   `AssetImageRef + source_rect + linear + multiply`，spawn marker 改为 `AssetImageRef + recolor`，不再做 Python
   crop/multiply/split/merge；resource/map/map_multi 当前双路对拍 3/3、双后端 warm parity 各 3/3 且 0 drift。
   map IR mem 从约 5.78 MiB 降至 465,152 B（约 -92.3%），map_multi 从约 16.68 MiB 降至 2,515,968 B
   （约 -85.6%）。
   **不能把“可显式选 linear”误读成“所有 `get_img_resized` 都可直接删”**：misc alias jacket 仍按
   原图→92 BILINEAR→padding 后 84 BICUBIC，birthday calendar icon 仍按原图→40 BILINEAR→padding 后
   32 BICUBIC。改成一次 lazy linear draw 会折叠历史双阶段内核，legacy baseline 实测分别漂移
   maxΔ34 / 1.118% 与 maxΔ21 / 4.196%，故两站刻意保留 eager。无 padding 的 birthday card thumbnail
   （80×80）与 profile X icon（24×24）则已用 `AssetImageRef + sampling="linear"` 替换，并对 main 逐像素一致。
...
| Lazy AssetRef | Done | 仓库内 builder 已迁到 `get_asset_image_ref`（card / gacha / event / score / vlive / misc / stamp / profile / inventory / music …）；`ImageBox`/`Painter.paste*` 以 `source_rect`/`sampling`/`ImageTint` 惰性表达 crop/resize/tint，production-local MySekai site/spawn/background 已同步。只有真实像素消费者与必须保留双阶段重采样的站点继续 eager。 |
...
| 测试与基准 | Done | 全量 Ruff、383 项 pytest、65 个可渲染 payload 冷态对拍与双后端 warm parity 已通过；另有 2 个 custom-profile no-payload 桶。 |
