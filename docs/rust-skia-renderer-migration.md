...
- **图片 lazy 边界已继续下沉**：`ImageBox` 与三种 `Painter.paste*` 共享 `source_rect`、显式 sampling 和
  ...
  point 仍是下一处可用现有原语消除的主要 mem raster。
...
2. 圆形头像/进度环、`concat_images`、mysekai harvest point 等确实读取像素的合成，以及为保持历史
   双阶段重采样而保留的 misc alias jacket / birthday calendar icon，仍会在 Python 侧产出或持有 PIL raster
   （条目 7 列出的刻意保留项）。
